...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://avaweiner@localhost/avawHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
class Gif(db.Model):
    ...
    def __repr__(self):
        return "{}\nFind it here: {}".format(self.title, self.embedURL)
...
def get_gifs_from_giphy(search_string):
    ...
    params_d = {'api_key':api_key, 'q':search_string, 'limit': 5}
    response = requests.get(baseurl, params = params_d)
...
def index():
    ...
    if request.method == "POST" and form.validate_on_submit():
        search = get_or_create_search_term(db.session, term=form.search.data)
...
def search_terms():
    ...
    all_terms = db.session.query(SearchTerm).all()
